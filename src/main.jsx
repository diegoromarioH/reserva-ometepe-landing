...
import'./styles.css';
import{initExternalAnalytics,trackEvent as trackBusinessEvent,trackPageView,recordFunnelStep}from'./services/analytics.service';
...
function toggleFavorite(slug){const cur=getFavorites();const next=cur.includes(slug)?cur.filter(s=>s!==slug):[...cur,slug];localStorage.setItem(FAV_KEY,JSON.stringify(next));favListeners.forEach(f=>f());return next.includes(slug)}
function useFavorite(slug){const[,force]=useState(0);useEffect(()=>{const f=()=>force(x=>x+1);favListeners.add(f);return()=>favListeners.delete(f)},[]);return isFavorite(slug)}
...
function Favorites({L,go}){setSeo('Tus alojamientos favoritos | Reserva Ometepe','Revisa los alojamientos que guardaste en Ometepe para comparar y solicitar disponibilidad.');useFavorite('__watch__');const saved=getFavorites();const hotels=DATA.hotels.filter(h=>saved.includes(h.slug));const benefits=useBenefits('accommodation');return <section className="page white"><div className="head"><p className="eyebrow">Tus favoritos</p><h1>Alojamientos que guardaste</h1><p>Compara tus opciones favoritas en Ometepe y solicita disponibilidad cuando estés listo. Se guardan solo en este dispositivo.</p></div>{hotels.length===0?<p className="ro-empty-favs">Aún no has guardado alojamientos. Toca el <Heart size={14}/> en cualquier tarjeta para guardarla aquí.</p>:<div className="ro-hcard-grid">{hotels.map(h=>{const benefit=findBenefit(benefits,'accommodation',h.slug);const low=h.available>0&&h.available<=3;return <article className="ro-hcard" key={h.id}>
      ...
      <div className="ro-hcard-body">
        ...
        <div className="ro-hcard-bottom">
          ...
          <div className="ro-hcard-actions"><button onClick={()=>go('/hotel/'+h.slug)} className="btn ghost small">Ver alojamiento</button><button onClick={()=>go('/reservar?hotel='+h.slug)} className="btn small">Solicitar</button></div>
        </div>
      </div>
    </article>})}</div>}<HotelCardStyle/></section>}
function NavFavButton({go,className='nav-fav'}){useFavorite('__watch__');const n=getFavorites().length;return <button className={className} onClick={()=>go('/favoritos')} aria-label="Ver favoritos"><Heart size={18} fill={n>0?'currentColor':'none'}/>{n>0&&<span className="nav-fav-count">{n}</span>}</button>;}
function Nav({L,go,menu,setMenu}){return <header className="nav"><button onClick={()=>go('/')} className="brand"><img src="/RO.png" alt="Reserva Ometepe"/><span><b>Reserva Ometepe</b><small>Planifica tu viaje</small></span></button><nav className={menu?'open':''}><button onClick={()=>go('/')}>{L.home}</button><button onClick={()=>go('/hoteles')}>{L.hotels}</button><button onClick={()=>go('/motos')}>{L.motos}</button><div className="drop"><button>{L.sched}<ChevronDown size={16}/></button><div><button onClick={()=>go('/horario-barcos')}>{L.barcos}</button><button onClick={()=>go('/transporte-terrestre')}>{L.transport}</button></div></div><div className="drop"><button>Experiencias<ChevronDown size={16}/></button><div><button onClick={()=>go('/guias')}>{L.guides}</button><button onClick={()=>go('/destinos')}>{L.destinations}</button><button onClick={()=>go('/actividades')}>{L.activities}</button><button onClick={()=>go('/blog')}>{L.blog}</button></div></div><button onClick={()=>go('/eventos-y-promociones')}>{L.events}</button><button onClick={()=>go('/contacto')}>{L.contact}</button></nav><NavFavButton go={go}/><button className="hamb" onClick={()=>setMenu(!menu)}>{menu?<X/>:<Menu/>}</button></header>}
function MobileIsland({L,go}){const[open,setOpen]=useState(false);useEffect(()=>{document.body.classList.toggle('mobile-menu-open',open);return()=>document.body.classList.remove('mobile-menu-open')},[open]);const nav=(p)=>{go(p);setOpen(false)};return <div className={open?'mobile-island open':'mobile-island'}><button className="island-trigger" onClick={()=>setOpen(!open)}><img src="/RO.png" alt="Reserva Ometepe"/><span>{open?'Cerrar':'Menú'}</span>{open?<X size={18}/>:<Menu size={18}/>}</button>{open&&<><div className="island-backdrop" onClick={()=>setOpen(false)}></div><div className="island-panel"><button onClick={()=>nav('/')}><Palmtree/>Inicio</button><button onClick={()=>nav('/hoteles')}><Hotel/>{L.hotels}</button><button onClick={()=>nav('/favoritos')}><Heart/>Favoritos</button><button onClick={()=>nav('/motos')}><Bike/>Motos</button><button onClick={()=>nav('/horario-barcos')}><Ship/>Barcos</button><button onClick={()=>nav('/transporte-terrestre')}><Bus/>Transporte</button><button onClick={()=>nav('/guias')}><Compass/>Guías</button><button onClick={()=>nav('/experiencias')}><Palmtree/>Experiencias</button><button onClick={()=>nav('/eventos-y-promociones')}><CalendarDays/>Eventos</button><button onClick={()=>nav('/contacto')}><Mail/>Contacto</button></div></>}</div>}
function SubscribePrompt({section='general',title='Recibe novedades de Ometepe',text='Déjanos tu correo para recibir guías, eventos y avisos útiles para planificar tu viaje.'}){const[show,setShow]=useState(false);const[done,setDone]=useState(false);const[email,setEmail]=useState('');const[sending,setSending]=useState(false);const[err,setErr]=useState(false);useEffect(()=>{const key='ro-sub-'+section;if(localStorage.getItem(key))return;const t=setTimeout(()=>setShow(true),900);return()=>clearTimeout(t)},[section]);if(!show||done)return null;const save=async(e)=>{e.preventDefault();setSending(true);setErr(false);try{await subscribeNewsletter({email,interest:section,source:section});localStorage.setItem('ro-sub-'+section,'1');trackEvent('newsletter_signup',{section});setDone(true)}catch{setErr(true)}finally{setSending(false)}};return <div className="sub-alert"><button className="sub-close" onClick={()=>{localStorage.setItem('ro-sub-'+section,'1');setShow(false)}}>×</button><div><Bell/><b>{title}</b><p>{text}</p></div><form onSubmit={save}><input required placeholder="Tu correo" type="email" value={email} onChange={e=>setEmail(e.target.value)}/><button disabled={sending}>{sending?'Enviando…':'Suscribirme'}</button></form>{err&&<small style={{color:'var(--red)',display:'block',marginTop:'8px'}}>No pudimos guardar tu correo. Intenta de nuevo.</small>}</div>}
...
`}</style>}
function ExperienceImg({src,Icon}){
...
function FavoriteButton({slug,className='ro-fav-btn'}){
  const fav=useFavorite(slug);
  return <button type="button" className={className+(fav?' active':'')} aria-label={fav?'Quitar de favoritos':'Guardar en favoritos'} onClick={e=>{e.preventDefault();e.stopPropagation();const nowFav=toggleFavorite(slug);trackEvent(nowFav?'add_favorite':'remove_favorite',{slug})}}>
...
function HotelGrid({L,go}){
  ...
  return <>
    ...
    <div className="filterbar hotel-filters" aria-label="Filtros de alojamientos">
      ...
      <label><span>Área</span><select value={zone} onChange={e=>setZone(e.target.value)}>{zones.map(z=><option key={z}>{z}</option>)}</select></label>
      ...
      <label><span>Ordenar</span><select value={sort} onChange={e=>setSort(e.target.value)}><option value="destacados">Destacados</option><option value="precio">Menor precio</option><option value="resena">Mejor reseña</option><option value="area">Área</option></select></label>
    </div>
...
.ro-fav-btn{width:30px;height:30px;bottom:8px;right:8px}
...
function ReservePage(){
  ...
  if(sent)return <section className="page white"><div className="ro-wizard-done"><CheckCircle2 size={48}/><h1>Solicitud enviada</h1><p>Recibimos tu solicitud. Revisaremos disponibilidad y te contactaremos por correo y WhatsApp con una propuesta.</p><a className="btn" href="/">Volver al inicio</a></div><ReserveWizardStyle/></section>;
...
function Footer({L,go}){return <footer><div><img className="footer-logo" src="/logo-white.png" alt="Reserva Ometepe"/><h3>Reserva Ometepe</h3><p>Guía local para planificar viajes a Isla de Ometepe: hoteles, barcos, transporte, actividades, eventos y experiencias.</p></div><div><b>Explorar</b><a href="/hoteles" onClick={e=>{e.preventDefault();go('/hoteles')}}>Alojamientos</a><a href="/horario-barcos" onClick={e=>{e.preventDefault();go('/horario-barcos')}}>Horario de barcos</a><a href="/destinos" onClick={e=>{e.preventDefault();go('/destinos')}}>Destinos</a><a href="/actividades" onClick={e=>{e.preventDefault();go('/actividades')}}>Actividades</a></div><div><b>Contacto</b><a href="mailto:alianzas@reservaometepe.com">alianzas@reservaometepe.com</a><a href="mailto:soporte@reservaometepe.com">soporte@reservaometepe.com</a></div></footer>}
